...
- **16 new lifecycle tests** in `tests/engine-lifecycle.test.js` pinning the contract: init runs before check; init runs once; dispose runs after check; dispose runs even on check throw; dispose runs even on init throw; dispose errors are swallowed (warn only); priority sorts descending; default priority is 0; ties preserve registration order; init crash skips check but still records BLOCK; non-Error init crash gets stringified; disabled-in-config guards do NOT have init / check / dispose called.
